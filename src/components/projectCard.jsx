...
border-${props => props.orientation === 0 ? "right" : "left"}: ${props => props.mediaQuery  ? "2px solid rgba(85, 60, 154, 0.5)" : "none"};
...
background:linear-gradient(0deg,rgba(238, 75, 43, 0.2), rgba(85, 60, 154, 0.2)); 
border-radius: 16px;
height: 80%;
width: 100%;
overflow: hidden;
...
function ProjectCard({orientation = 0, projectTitle, projectCategory, description, techUsed, codeLink, demoSrc, demoLink, projectLink}){
const [mediaQuery, setMediaQuery] = useState(window.matchMedia("(min-width: 1024px)").matches);
...
const listProjectCategory = projectCategory.map((cat) => <ProjectCategory key={cat}>{cat}</ProjectCategory>);
const listTechUsed = techUsed.map((tech) => <IconDiv key={tech} alt= {tech.alt} title={tech.title} src={tech.src} />);
...
return(
    ...
    <A href={codeLink}>Code <FiGithub/></A>
    <A href={demoLink}>Demo <BiWindowOpen/></A>
    </div>
    ...
    <A orientation={orientation} href={projectLink}>Learn More <AiOutlineArrowRight/></A>
